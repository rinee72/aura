...
/// WP-3.4: 셀럽 프로필 관리
/// 
/// 셀럽이 자신의 프로필 정보를 관리할 수 있는 서비스입니다.
class CelebrityProfileService {
  /// 내 프로필 조회
  /// 
  ...
  static Future<UserModel> getMyProfile() async {
    try {
      ...
      try {
        ...
        throw Exception('프로필 관리는 셀럽만 가능합니다.');
      }
  ...
  static Future<UserModel> updateProfile({
    ...
    try {
      ...
      try {
        ...
        throw Exception('프로필 수정은 셀럽만 가능합니다.');
      }
  ...
  /// 
  /// 참고: Supabase Flutter SDK는 업로드 진행률 콜백을 직접 지원하지 않으므로,
  /// 업로드 중임을 표시하는 로딩 인디케이터를 사용합니다.
  static Future<String> updateProfileImage(File imageFile) async {
    try {
      ...
      // 권한 검증: 셀럽만 프로필 이미지 업로드 가능
      try {
        PermissionChecker.requireRole(user, PermissionChecker.roleCelebrity);
      } catch (e) {
        throw Exception('프로필 이미지 업로드는 셀럽만 가능합니다.');
      }
  ...
  /// 구독자 수 조회
  /// 
  /// Returns: 현재 사용자의 구독자 수
  /// Throws: 권한 없음, 구독자 수 조회 실패 시
  static Future<int> getSubscriberCount() async {
    try {
      final client = SupabaseConfig.client;
      final currentUser = client.auth.currentUser;
      ...
      // 권한 검증: 셀럽만 구독자 수 조회 가능
      try {
        PermissionChecker.requireRole(user, PermissionChecker.roleCelebrity);
      } catch (e) {
        throw Exception('구독자 수 조회는 셀럽만 가능합니다.');
      }

      // 구독자 수 조회
      final count = await SubscriptionService.getSubscriberCount(user.id);

      print('✅ 구독자 수 조회 성공: ${user.id} (구독자: $count명)');
      return count;
    } catch (e) {
      print('❌ 구독자 수 조회 실패: $e');
      rethrow;
  ...
  static Future<void> deleteProfileImage() async {
    try {
      ...
      // 권한 검증: 셀럽만 프로필 이미지 삭제 가능
      try {
        PermissionChecker.requireRole(user, PermissionChecker.roleCelebrity);
      } catch (e) {
        throw Exception('프로필 이미지 삭제는 셀럽만 가능합니다.');
      }
